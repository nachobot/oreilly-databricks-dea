...
(spark.readStream
           ...
           .selectExpr("*","_metadata.file_path as source_file","current_timestamp() as arrival_time") 
     .writeStream
           .format("delta")
...
# MAGIC %md
# MAGIC it is considered a best practice to embed file metadata information into your Bronze layer when ingesting from files in cloud storage, so that we can know which file each row of our raw data came from.
# MAGIC Chain selectExpr to perform transformations on the incoming data.
...
# MAGIC %sql
# MAGIC DESCRIBE enrollments_bronze
